...
class MobileVitBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Calculate padding needed to make dimensions divisible by 2
        pad_h = (2 - h % 2) % 2
        pad_w = (2 - w % 2) % 2

        # Add padding if needed (padding: left, right, top, bottom)
        if pad_h > 0 or pad_w > 0:
            local_repr_padded = torch.nn.functional.pad(local_repr, (0, pad_w, 0, pad_h))
        else:
            local_repr_padded = local_repr

        _, _, h_pad, w_pad = local_repr_padded.shape

        # Apply patch-based transformation
        global_repr = rearrange(local_repr_padded, 'b d (h ph) (w pw) -> b (ph pw) (h w) d', ph=2, pw=2)
        global_repr = self.transformer(global_repr)
        global_repr = rearrange(global_repr, 'b (ph pw) (h w) d -> b d (h ph) (w pw)',
                                h=h_pad // 2, w=w_pad // 2, ph=2, pw=2)
...
class MobileViT(nn.Module):
    def __init__(self, img_size, features_list, d_list, transformer_depth, expansion, num_classes = 200):
    ...
    def forward(self, x, state):
        ...
        # Classification head
        output = self.fc(x)
        if state[6]:  # Note: index adjusted from 7 to 6 since we removed one checkpoint
...
def mobilevit_xxs(img_size = 224, num_classes = 200):
